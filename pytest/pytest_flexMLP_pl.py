...
from stfs_pytoolbox.ML_Utils.TabularLoader import TabularLoader
from stfs_pytoolbox.ML_Utils.flexMLP_pl import flexMLP_pl
...
class TestClass:
    ...
    def test_prepare_data(self):
        ...
        with pytest.raises(KeyError):
            ...
            model01 = flexMLP_pl(hparams=hparams, TabularLoader=Loader)
            trainer01 = pl.Trainer(fast_dev_run=True)
            trainer01.fit(model01)
        with pytest.raises(KeyError):
            Loader = self.get_loader()
            Loader.samples_val = None
            model02 = flexMLP_pl(hparams=hparams, TabularLoader=Loader)
            trainer02 = pl.Trainer(fast_dev_run=True)
            trainer02.fit(model02)
        with pytest.raises(KeyError):
            Loader = self.get_loader()
            Loader.samples_test = None
            model03 = flexMLP_pl(hparams=hparams, TabularLoader=Loader)
