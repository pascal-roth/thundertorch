...
1. The data samples to train and test the network included in a
   `DataLoader <./DataLoader.ipynb>`__
2. The `Model <./Models.ipynb>`__ itself
3. The `Trainer <./Trainer.ipynb>`__ that takes the
   `Model <./Models.ipynb>`__ and trains, as well as test it with the
   data included in the `DataLoader <./DataLoader.ipynb>`__
...
-  by using the yaml interface
   ...
   -  for an arbriary number of models as a queue of parallel processes
      ...
      `here <./MultiModelTraining.ipynb>`__
